...
files = [
    r'c:\Users\srini\Dexter-bmscehack\web\src\pages\Inference.jsx',
    r'c:\Users\srini\Dexter-bmscehack\web\src\pages\Visualization.jsx'
]
...
for path in files:
    ...
    code = re.sub(r'className=\"btn btn-success(.*?)\"', r'className=\"shdcn-button shdcn-button-primary\1\"', code)
    code = re.sub(r'className=\"btn btn-danger(.*?)\"', r'className=\"shdcn-button shdcn-button-primary\1\"', code)
    ...
    code = re.sub(r'className=\"data-table\"', r'className=\"shdcn-table\"', code)
    code = re.sub(r'className=\"badge badge-(\w+)\"', r'className=\"shdcn-badge\"', code)
    ...
    code = code.replace('"rgba(200,216,240,0.7)"', '"#737373"')
    ...
    code = code.replace('"#2dd4bf"', '"var(--primary)"')
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(code)
    print(f'Done format update on {path}')
